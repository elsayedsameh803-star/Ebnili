...
- `src/lib/gemini.ts` يتولّى الاتصال المباشر بـ Gemini من المتصفح عبر `@google/genai`
  (موديلات `gemini-3.8-flash` ثم `gemini-3.6-flash` كاحتياطي، مع تنظيف الأسوار البرمجية).
- `src/lib/db.ts` طبقة تخزين مزدوجة: Supabase عند توفّر مفاتيح صحيحة، وإلا تخزين المتصفح —
  وهي التي تمنع فشل التوليد عند غياب الخلفية (سبب أخطاء 404 سابقًا).
- `src/App.tsx` + `ChatPanel` + `LivePreview` تخطيط بأسلوب Bolt.new: محادثة يسار، معاينة حية يمين.
